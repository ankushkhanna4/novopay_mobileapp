...
public class RBLStatusEnquiryPage extends BasePage {
	DBUtils dbUtils = new DBUtils();

	WebDriverWait wait = new WebDriverWait(mdriver, 30);
	WebDriverWait waitLess = new WebDriverWait(mdriver, 5);
	...
	public RBLStatusEnquiryPage(AndroidDriver<MobileElement> mdriver) {
		super(mdriver);
...
//				Assert.assertEquals("No Transaction available", toasterMsg.getText());
	...
	public void assertionOnTxnScreen(Map<String, String> usrData)
		...
		if (!usrData.get("STATUS").equalsIgnoreCase("Auto-Refunded")) {
			...
			try {
				Assert.assertEquals(seAmount.getText(), "amount : ₹ " + txnDetailsFromIni("GetFailAmount", "") + ".00");
			} catch (Exception e) {
				Assert.assertEquals(enquiryAmount.getText(),
						"amount : ₹ " + txnDetailsFromIni("GetFailAmount", "") + ".00");
			}
			Log.info(seAmount.getText());
		}
		try {
