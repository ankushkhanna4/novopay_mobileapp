...
public class RBLMoneyTransferPage extends BasePage {
	...
	CommonUtils commonUtils = new CommonUtils(mdriver);
	...
	@AndroidFindBy(id = "in.novopay.merchant:id/autoCompleteTextView")
	...
	@AndroidFindBy(id = "in.novopay.merchant:id/textViewTitle")
	MobileElement featureTitle;

	// Load all objects
	public RBLMoneyTransferPage(AndroidDriver<MobileElement> mdriver) {
		super(mdriver);
...
//				Log.info("Bene mobile number '" + getBeneNumberFromIni("GetNum") + "' entered");
				...
				for (int i = 0; i < 10; i++) {
					try {
						beneXpath = ".//android.widget.RelativeLayout[@index='" + i
								+ "']/android.widget.LinearLayout/android.widget.TextView[contains(@text,'" + beneName
								+ "')]/following-sibling::android.widget.TextView[contains(@text,'" + beneACNum + ", "
								+ dbUtils.getBank(beneIFSC) + "')]";
						break;
					} catch (Exception e) {
						System.out.println("Bene path not found for " + i + ", continuing the search");
						continue;
					}
				}
				...
				if (usrData.get("ASSERTION").equalsIgnoreCase("Bene Limit")) {
					...
					if (usrData.get("ADDBENE").equalsIgnoreCase("Indirectly")) {
						...
						if (usrData.get("CUSTOMERNUMBER").equalsIgnoreCase("NewNum")
								|| usrData.get("ASSERTION").equalsIgnoreCase("Other Partner")) {
	...
	public void moneyTransfer(Map<String, String> usrData)
			throws ClassNotFoundException, InterruptedException, ParseException {
		try {
			waitIFSC.until(ExpectedConditions.visibilityOf(enterMPIN));
		} catch (Exception e) {
			Log.info("entering IFSC code again");
			ifscSearchScreen(usrData, "code");
			wait.until(ExpectedConditions.elementToBeClickable(submitBtn));
	...
	public void assertionOnFailedScreen(Map<String, String> usrData)
		...
		if (usrData.get("ASSERTION").equalsIgnoreCase("Insufficient balance")) {
			...
			Assert.assertEquals(remittanceTxnFailScreenMessage.getText(),
					"BENEFICIARY: Minimum of ₹ 10.00 should be used as transaction amount");
		} else {
			Assert.assertEquals(remittanceTxnFailScreenMessage.getText(),
				...
				if (j == 3) {
					if (rowCount == 1) {
						if (remittanceTxnScreen.getAttribute("text").equalsIgnoreCase("success!")
							...
							tableStatusXpath = xpathHead + "/android.widget.LinearLayout/android.widget.TextView";
						} else if (remittanceTxnScreen.getAttribute("text").equalsIgnoreCase("failed!")
								|| remittanceTxnSuccessScreenMessage.getAttribute("text").contains("processing")) {
					...
					if (remittanceTxnScreen.getAttribute("text").equalsIgnoreCase("success!")
							&& !remittanceTxnSuccessScreenMessage.getAttribute("text").contains("processing")) {
						System.out.print(replaceSymbols(tableStatus.getText()));
	...
	public void otpScreen(Map<String, String> usrData)
			throws ClassNotFoundException, InterruptedException, ParseException {
		try {
			waitIFSC.until(ExpectedConditions.elementToBeClickable(otpTextField));
		} catch (Exception e) {
			Log.info("entering IFSC code again");
			ifscSearchScreen(usrData, "code");
			wait.until(ExpectedConditions.elementToBeClickable(submitBtn));
	...
	public void assertionOnFCM(Map<String, String> usrData) throws ClassNotFoundException {
		...
		String failFCMContent = "Customer: " + getCustomerDetailsFromIni("ExistingNum") + " Money Transfer of INR "
		...
		for (j = 1; j <= count; j++) {
			...
			case "Failed FCM":
				if (j == 1) {
					...
					Assert.assertEquals(fcmContentString(fcmContent.getText(), 0, 20, 21, 141, 0, 0), failFCMContent);
				}
				break;
			case "Queued FCM":
				if (j == 1) {
			...
			case "BeneSuccess FCM":
				if (j == 1) {
					...
					Assert.assertEquals(fcmContentString(fcmContent.getText(), 0, 21, 22, 146, 147, 175),
